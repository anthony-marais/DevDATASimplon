...
JOIN film_actor ON film_actor.film.category_id = film_category.category_id
JOIN film_category ON film_category.film_id = film.film_id
...
JOIN rental 
ON film.film_id = inventory.film_id AND inventory.inventory_id = rental.inventory_id
GROUP BY film.title HAVING count(rental.return_date) >= 10 ORDER BY count(rental.return_date) DESC;
